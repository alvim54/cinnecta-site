...
import englishVideo from '../../assets/videos/intro-english.mp4';
...
export class IndexPageTemplate extends Component {
  ...
  toggleVideoLanguage() {
    if(this.state.videoLanguage === 'en') {
      this.videoPlayer.src(video);
      this.setState({ videoLanguage: 'pt-br' })
    } else {
      this.videoPlayer.src(englishVideo);
      this.setState({ videoLanguage: 'en' })
    }
    this.videoPlayer.play();
  }

  render() {
    const { banner, featured, solutions } = this.props;
            ...
            <div className="grid-container">
              <div className="grid-x grid-margin-x">
                <div className="cell small-12">
                  <button className="video-button" onClick={() => this.toggleVideoLanguage()}>{this.state.videoLanguage === 'en' ? 'Assista em português' : 'Watch in english' }</button>
